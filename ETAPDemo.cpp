// ETAPDemo.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CETAPDemoApp, CFOProjectApp)
	//{{AFX_MSG_MAP(CETAPDemoApp)
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CETAPDemoApp object

CETAPDemoApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CETAPDemoApp initialization

BOOL CETAPDemoApp::InitInstance()
{
	...
	FOEnableAroundCenterLink(FALSE);
	FOEnableDefStyle(FALSE);
	FOEnableXPStyle(TRUE);
	gfxData.InitGUIType(GUI_BLUE);
	FOPEnableLinkRoute(FALSE);
	gfxData.m_bAutoSaveData = TRUE;
	//	m_drawlist.CreateX(IDB_BITMAP_ACTION, RGB(255,255,255) );
	gfxData.m_bWithPinNumberShow = FALSE;
	gfxData.m_bWithPinNameShow = FALSE;
	gfxData.m_bWithPortIndexShow = FALSE;
	gfxData.m_bWithCADDrawing = FALSE;
	gfxData.m_nDefCanvasExpand = 10;
	//	gfxData.m_bAroundCenterLink = FALSE;
	gfxData.m_strDefToolboxFile = _T("mytest.dat");
	gfxData.m_bWithContinuePan = FALSE;
	gfxData.m_bWithPanningRunTime = FALSE;
	gfxData.m_bWithAddiShowing = TRUE;
	gfxData.m_nDefCompLabelSpace = 12;
	gfxData.m_nLinkOutSideSize = 0;
	gfxData.m_bfoLinkOnLink = TRUE;
	gfxData.m_bWithStyle = FALSE;
	gfxData.m_crHighLight = RGB(255,0,0);
	gfxData.m_bEnableSimpleTrack = FALSE;
	gfxData.m_bWithFixRotateLabel = FALSE;
	gfxData.m_bWithAutoScrolling = FALSE;
	gfxData.fo_WithSpecifySelectColorMode = TRUE;
	m_drawlist.CreateX(IDB_BITMAP_ACTION, RGB(255,255,255) );
	gfxData.m_nDefNearestPortDist = 10;
	gfxData.m_nDefCompLabelSpace = 12;
	gfxData.m_nDefFontSize = 8;
	AfxInitRichEdit();
	...
	SetRegistryKey(_T("ucancode\\etapdemo"));
	LoadSettings ();
	...
	FO_SetRegKey(_T(".pss"), strFileTypeId);
	FO_SetRegKey(_T(".psp"), strFileTypeId);
	...
	_tcscpy( tmppath , _T(".pss\\OpenWithList\\"));_tcscat( tmppath ,sname);
	FO_WriteRegStringCreateValue(HKEY_CLASSES_ROOT,tmppath,NULL,NULL);
	
	_tcscpy( tmppath , _T(".psp\\OpenWithList\\"));_tcscat( tmppath ,sname);
	FO_WriteRegStringCreateValue(HKEY_CLASSES_ROOT,tmppath,NULL,NULL);
...
void CETAPDemoApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

CString CETAPDemoApp::XGetBaseFileName(const CString& strFileName)
{
	CString FileName = XGetFileName(strFileName);
	CString Ext = XGetExtension(FileName);
...
CETAPDemoView *CETAPDemoApp::FindProjectFilesView(CString strFileName)
{
	...
	while (pos != NULL)
	{
		CETAPDemoDoc* pDoc = (CETAPDemoDoc*)m_pDocTemplate->GetNextDoc(pos);
		
		CString str1 = pDoc->GetViewTitle();
		if ( CFODrawHelper::IsTwoStringEq(str1, strFileName ))
			return (CETAPDemoView *)pDoc->GetCurView();
	}
...
/////////////////////////////////////////////////////////////////////////////
// CETAPDemoApp message handlers


void CETAPDemoApp::MakeProjectPath(UINT nType)
{	
	XFOPMakeDirectory(m_strPath);
	CString string;
	
	string = m_strPath +_T("pssFiles");
...
CDocument* CETAPDemoApp::OpenDocumentFile(LPCTSTR lpszFileName)
{
	CString strExt;
...
void CETAPDemoApp::OnNewProject() 
{
...
//	Dlg.AddPage(&m_Page2, CNewProjectPage2::IDD);
	...
	if (Dlg.DoModal() == ID_WIZFINISH)
	{
		...
		{
			CString string;
			string = m_strPath +strProjectName + _T(".psp");
			m_pObj->SaveDocument(string);
			CString strText;
			strText = strProjectName + _T(" - ETAPDemo Editor");
			...
			{
				CString strPath1 = theApp.m_strPath +_T("pssFiles");
				CString strFile = strPath1 + _T("\\Main.pss");
				CExtTabModelManager m_model;
				m_model.InsertTab();
				m_model.SaveDocument(strFile);
				theApp.m_pObj->strMainApp = _T("Main.pss");
			}
...
void CETAPDemoApp::OnUpdateNewProject(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	
}

void CETAPDemoApp::OnOpenProject() 
{
	// TODO: Add your command handler code here
	CMainFrame *pParent = STATIC_DOWNCAST(CMainFrame,AfxGetMainWnd());
	ASSERT_VALID(pParent);
	static char szFilter[]="ETAPDemo Project(.psp)|*.psp||";
...
void CETAPDemoApp::OnUpdateOpenProject(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	
}

void CETAPDemoApp::OnSaveProject() 
{
	// TODO: Add your command handler code here
	CMainFrame *pParent = STATIC_DOWNCAST(CMainFrame,AfxGetMainWnd());
	ASSERT_VALID(pParent);
	...
	{
		CString string;
		string = m_strPath +strProjectName +".psp";
		m_pObj->SaveDocument(string);
...
void CETAPDemoApp::OnUpdateSaveProject(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(!strProjectName.IsEmpty());
}

void CETAPDemoApp::OnCloseProject() 
{
	// TODO: Add your command handler code here
	CMainFrame *pParent = STATIC_DOWNCAST(CMainFrame,AfxGetMainWnd());
	ASSERT_VALID(pParent);
...
void CETAPDemoApp::OnUpdateCloseProject(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(!strProjectName.IsEmpty());
}

void CETAPDemoApp::OnSaveAll() 
{
	// TODO: Add your command handler code here
	CMainFrame *pParent = STATIC_DOWNCAST(CMainFrame,AfxGetMainWnd());
	ASSERT_VALID(pParent);
...
void CETAPDemoApp::OnUpdateSaveAll(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
...
void CETAPDemoApp::OnFileNew() 
{
	// TODO: Add your command handler code here
	m_pDocTemplate->OpenDocumentFile(NULL);
}

void CETAPDemoApp::DoSaveSetting()
{
	CReg reg;
...
void CETAPDemoApp::DoLoadSetting()
{
	DWORD dTest = 0;
	CReg reg;
...
void CETAPDemoApp::OnFileOpen() 
{
	// TODO: Add your command handler code here
	CString title;
	
	static char szFilter[] = _T("All Format (.pss;.psp;.scp)|*.pss;*.psp;*.scp|Screen Files(.pss)|*.pss|Script Files (.scp)|*.scp|Project Files(.psp)|*.psp|All Files(*.*)|*.*||");
	title = _T("ETAPDemo Editor");
...
void CETAPDemoApp::OnNewScript() 
{
	// TODO: Add your command handler code here
	pCodeTemplate->OpenDocumentFile(NULL);
}

void CETAPDemoApp::OnOpenSamples() 
{
	// TODO: Add your command handler code here
...
*/
BOOL CNewImageList::CreateX(UINT nID)
{
	...
	// height and width are equal for each image, thus the total width of the bitmap
	// must be a multiple of its height
	ASSERT(bmp.bmWidth%bmp.bmHeight == 0);
	
	return CreateX(nID, bmp.bmHeight, bmp.bmHeight, RGB(255,255,255));
}
...
transparent color = white
*/
BOOL CNewImageList::CreateX(UINT nID, int cx)
{
	return CreateX(nID, cx, cx, RGB(255,255,255));
}
...
*/
BOOL CNewImageList::CreateX(UINT nID, int cx, COLORREF colMask)
{
	return CreateX(nID, cx, cx, colMask);
}
...
transparent color = white
*/
BOOL CNewImageList::CreateX(UINT nID, int cx, int cy)
{
	return CreateX(nID, cx, cy, RGB(255,255,255));
}
...
transparent color = colMask
*/
BOOL CNewImageList::CreateX(UINT nID, COLORREF colMask)
{
	CBitmap res;
	if(!res.LoadBitmap(nID))
		return FALSE;
	...
	return CreateX(nID, bmp.bmHeight, bmp.bmHeight, colMask);
}
...
transparent color = colMask
*/
BOOL CNewImageList::CreateX(UINT nID, int cx, int cy, COLORREF colMask)
{
